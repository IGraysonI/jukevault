package com.example.jukevault_android.providers
...
/**
 * A singleton used to define all variables/methods that will be used by the plugin.
 *
 * The singleton will provide the ability to 'request' required variables/methods on
 * any moment.
 *
 * All variables/methods should be defined after plugin initialization (activity/context)
 * and dart request (call/result).
 */
object PluginProvider {
    private const val ERROR_MESSAGE =
        "Tried to get one of the methods, but the 'PluginProvider' has not been initialized."
    ...
    /**
     * The current dart request [MethodCall]. Will be replaced with newest dart request.
     *
     * @throws UninitializedPluginProviderException
     * @return [MethodCall]
     */
    ...
    /**
     * The current dart request [MethodChannel.Result]. Will be replaced with newest dart request.
     *
     * @throws UninitializedPluginProviderException
    ...
    class UninitializedPluginProviderException(message: String) : Exception(message)
}