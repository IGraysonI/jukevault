...
class AlbumsQuery {
  ...
  Future<List<AlbumModel>> queryAlbums(
    ...
    switch (filter.albumSortType) {
      case AlbumSortTypeEnum.ALBUM:
        albums.sort(
          (v1, v2) => v1.album.isCase(filter!.ignoreCase).compareTo(
                v2.album.isCase(filter.ignoreCase),
              ),
        );
        break;

      case AlbumSortTypeEnum.ARTIST:
        albums.sort(
      ...
      case AlbumSortTypeEnum.NUM_OF_SONGS:
        albums.sort(
  ...
  Future<Map<String, Object?>> _formatAlbum(Map album, String data) async {
    ...
    int numOfAudios = _audios.where((audio) {
      if (audio.album != null && audio.album!.isNotEmpty) return audio.album! == album["Album"];
      return false;
    }).length;
