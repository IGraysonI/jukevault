...
class AudiosQuery {
  ...
  Future<List<AudioModel>> queryAudios({
    ...
    switch (filter.audioSortType) {
      case AudioSortTypeEnum.TITLE:
        _audios.sort((v1, v2) => v1.title.compareTo(v2.title));
        break;

      case AudioSortTypeEnum.ARTIST:
        _audios.sort(
          (v1, v2) => v1.artist.orEmpty.isCase(filter!.ignoreCase).compareTo(
      ...
      case AudioSortTypeEnum.ALBUM:
        _audios.sort(
          (v1, v2) => v1.album.orEmpty.isCase(filter!.ignoreCase).compareTo(
                v2.album.orEmpty.isCase(filter.ignoreCase),
              ),
        );
        break;

      case AudioSortTypeEnum.SIZE:
        _audios.sort((v1, v2) => v1.size.compareTo(v2.size));
        break;

      case AudioSortTypeEnum.DISPLAY_NAME:
        _audios.sort(
          (v1, v2) => v1.displayName.isCase(filter!.ignoreCase).compareTo(
  ...
  Map<String, Object?> _formatAudio(Map audio, String data, int size) {
    String? fExtension = path.extension(data);
    return {
