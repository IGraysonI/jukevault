...
class ArtistsQuery {
  ...
  // Artist projection.
  List<String?> projection = ["_id", "artists", "number_of_albums", "number_of_tracks"];
  ...
  Future<List<ArtistModel>> queryArtists(
    ...
    List<Map<String, Object?>> computedArtists = await compute(_fetchListOfArtists, instances);
    ...
    switch (filter.artistSortType) {
      case ArtistSortTypeEnum.ARTIST:
        albums.sort(
          (v1, v2) => v1.artist.isCase(filter!.ignoreCase).compareTo(
                v2.artist.isCase(filter.ignoreCase),
              ),
        );
        break;

      case ArtistSortTypeEnum.NUM_OF_TRACKS:
        albums.sort((v1, v2) {
      ...
      case ArtistSortTypeEnum.NUM_OF_ALBUMS:
        albums.sort((v1, v2) {
  ...
  Future<Map<String, Object?>> _formatArtist(Map artist, String data) async {
    ...
    int numOfAudios = _audios.where((audio) {
      if (audio.artist != null && audio.artist!.isNotEmpty) return audio.artist! == artist["Artist"];
      return false;
    }).length;
