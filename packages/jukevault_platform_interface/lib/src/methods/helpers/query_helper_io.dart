...
class QueryHelper extends QueryHelperInterface {
    ...
    if (fromAsset ?? false) {
      ...
      List<File> directoryEntities = (fromAppDir != null && fromAppDir)
          ? await getApplicationSupportDirectory().then((dir) {
              return dir.listSync(followLinks: lookSubs).whereType<File>().toList();
            })
          // The defaultDirectory on windows: 'C:\Users\user\Music'.
          : _defaultDirectory.listSync(recursive: lookSubs).whereType<File>().toList();

      // Get only [mp3] files.
      var mp3Files = directoryEntities.where((file) => file.path.endsWith('.mp3'));
  ...
  Future<String?> saveArtworks({
    ...
    // directory.
    Directory dirPath = temporary ? await getTemporaryDirectory() : await getApplicationSupportDirectory();
    ...
    // [type].
    File artFile = File('${dirPath.path + defaultArtworksPath}\\$id$fileType');
  ...
  Future<ArtworkModel?> getCachedArtwork({
    ...
    // directory.
    Directory dir = temporary ? await getTemporaryDirectory() : await getApplicationSupportDirectory();
    ...
    File? art;
    try {
      art = artworksDir.listSync(recursive: true).whereType<File>().firstWhere((f) => f.path.contains('$id'));
    } catch (e) {
      return null;
    }
    ...
    if (art.existsSync()) {
      return ArtworkModel(
          {'_id': id, 'artwork': await art.readAsBytes(), 'path': art.path, 'type': art.uri.pathSegments.last});
    }
