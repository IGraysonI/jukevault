  ...
  Future<List<GenreModel>> queryGenres({
    ...
    switch (filter.genreSortType) {
      case GenreSortType.GENRE:
        genres.sort(
            (val1, val2) => val1.genre.isCase(filter!.ignoreCase).compareTo(val2.genre.isCase(filter.ignoreCase)));
        break;
    ...
    for (var mp3instance in instances) {
      mp3instance.forEach((key, value) async {
        if (key == 'mp3' && (value as MP3Instance).parseTagsSync()) {
          ...
          int count = mediaCount.putIfAbsent(genre, () => mediaCount[genre] = 0);
          ...
          if (hList.contains(genre)) {
            int index = listOfGenres.indexWhere((tGenre) => tGenre['name'] == genre);
